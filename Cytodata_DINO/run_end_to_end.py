...
from utils import FocalBCELoss, threshold_output
from utils import write_to_tensorboard
from utils import get_scheduler, get_optimizer
from utils import init_distributed_mode, get_params_groups
...
def run_end_to_end(
    ...
    **kwargs,
    ...
    if type(args.prepare_kaggle_submission) == str:
        args.prepare_kaggle_submission = args.prepare_kaggle_submission == "True"
...
def test_model(feature_extractor, classifier, dl, epoch, device, criterion, args):
    ...
    with torch.no_grad():
        predictions, all_targets, losses = [], [], []
        batch_pbar = tqdm(
            enumerate(dl),
            total=len(dl),
            unit=" test batches",
...
def run_training(
    ...
    if is_main_process():
        ...
        save_model(
            classifier,
            f"{args.output_dir}/{args.output_prefix}/classifier_final.pth",
            args,
        )
