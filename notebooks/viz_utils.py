...
class DatasetFields:
    CellId = "CellId"
    CellImagePath = "registered_path"
    CellFeature = "NUC_shape_volume"
...
def _save_plot(
    ...
    for row_index, row in dataset.iterrows():
        ...
        # Read 3D Image
        if 'projection' not in DatasetFields.CellImagePath:
            img = read_ome_zarr(row[DatasetFields.CellImagePath])
        ...
        # Read Projected IMage
            ...
            img = AICSImage(row[DatasetFields.CellImagePath])
            chan_names = [img.channel_names[i] for i in channels] 
            img_data = img.get_image_data("CZYX", C=channels)
            img_data = rescale_image(img_data, chan_names)
...
def _collect_group(
    ...
    try:
        ...
        if row[str(metadata)] or row[str(metadata)] == 0:
            assert DatasetFields.CellImagePath in row.index
            save_path_index = int(
...
def diagnostic_sheet(
    ...
    proj_method: str = "max", 
    metadata: Optional[Union[list, str]] = DatasetFields.StructureName,
    feature: Optional[str] = None,
    fig_width: Optional[int] = None,
    fig_height: Optional[int] = None,
    ...
    if metadata:
        ...
        with DistributedHandler(distributed_executor_address) as handler:
            ...
            handler.batched_map(
                ...
                batch_size=len(all_grouped_datasets),
            )
